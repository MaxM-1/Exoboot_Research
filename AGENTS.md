...
- **Perception staircase varies `t_peak` only.** Rise and fall durations are coupled (`t_rise = t_peak - T_ACT_START`, `t_fall = T_ACT_END - t_peak`). Actuation start (26.0 %) and end (61.6 %) are **constants of the experimental design** — do not let either drift. The legacy `RISE_TIME_TEST` / `FALL_TIME_TEST` modes are aliases of `PEAK_TIME_TEST` for back-compat only. See Session 6 in [sequential_change_log.md](sequential_change_log.md).
