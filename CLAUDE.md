...
Peak-time perception experiment for the Dephy ExoBoot ankle exoskeleton. The participant walks in two exoboots while an adaptive staircase protocol varies the **peak time** of the Collins torque profile to find their Just Noticeable Difference (JND). Rise and fall durations are coupled to peak time so that **actuation start (26.0 %) and actuation end (61.6 %) of the gait cycle stay constant** — only the peak slides between them. This is adapted from Xiangyu Peng's actuation-timing perception research (same protocol structure: A/B comparison, 25 % catch trials, up–down staircase, ~9 sweeps per approach direction). Rise-time-only and fall-time-only modes from earlier drafts have been removed (Session 6) because they produced a flat plateau in the torque curve.
...
- **perception_test.py** — `PerceptionExperiment` class. Manages both boots, runs familiarization (manual `t_peak` adjustment) and the perception staircase. Single staircase variable is `t_peak`; rise and fall are derived (`t_rise = t_peak - T_ACT_START`, `t_fall = T_ACT_END - t_peak`). Handles trial sequencing, catch trials (25 % rate), rest/warm-up strides, CSV logging, and emits richer status messages to the GUI: `condition_announce`, `catch_flag`, `trial_phase`, `stride_progress`, `profile_preview`. Static helpers `_clamp_peak` and `_collins_curve` are also used by [DataLog/analysis/perception_plots.py](DataLog/analysis/perception_plots.py).
- **gui.py** — `ExperimentGUI` class (PyQt5 `QMainWindow`). Setup inputs (participant ID, weight, ports, approach direction — from above / from below), control buttons, real-time status display with: large condition banner, color-coded phase indicator (warm-up / Timing A / Timing B / RESPOND / Rest), stride counter, sweep/trial progress, reference and comparison `t_peak`, embedded matplotlib torque-profile preview (`FigureCanvasQTAgg`), and an experimenter-only `CATCH TRIAL` tag. Polls the experiment status queue via a 50 ms `QTimer`. Safe `closeEvent` stops motors before exit with a 3-second fallback timer. All experiment logic is delegated to `PerceptionExperiment`.
...
- **Adaptive staircase**: Single variable is `t_peak` (% gait). Reference `T_PEAK_REF = 51.3 %`. Initial comparison at ±3 % from reference (sign chosen by from-above / from-below radio). Step size 1 %. 25 % catch trials. 10 strides per trial (5 Timing A + 5 Timing B, A/B order randomised). Up to 55 trials or 9 sweeps. Reversal counted on response polarity change. JND analysis is downstream and not implemented in this codebase — only diagnostic plots.
