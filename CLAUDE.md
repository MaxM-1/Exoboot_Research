...
- **exo_off_tests/** — Standalone *sensors-only* tooling for heel-strike-threshold tuning. [`exo_off_tests/exo_off_recorder.py`](exo_off_tests/exo_off_recorder.py) opens both boots, streams at 100 Hz with motors commanding zero current (no torque), and logs the standard per-iter `ExoLogger` CSV plus a combined L/R merged CSV. Forbidden from calling `cur_ramp_up/down`, `set_motor_current`, `cal_motor_offset`, `zero_boot`, or `init_collins_profile`. [`exo_off_tests/exo_off_analysis.py`](exo_off_tests/exo_off_analysis.py) plots gyroZ + heel-strike markers + threshold lines + ankle angle + stride-duration histogram, and prints suggested per-speed `HEELSTRIKE_THRESHOLD_ABOVE/BELOW` values from observed stride peaks. Use this when the user reports inconsistent heel-strike detection across treadmill speeds (see Session 8 in [sequential_change_log.md](sequential_change_log.md)).
